...
class PrestamoController extends BaseController
{
    public function index(Request $request)
    {
        try {
            ...
            $prestamos = Prestamo::with(['usuario', 'libro.autores'])
                ->orderBy('created_at', 'desc');

            // Aplicar filtros
            if ($estado) {
                $prestamos->where('estado', $estado);
            }

            if ($usuario_id) {
                $prestamos->where('usuario_id', $usuario_id);
            }

            if ($libro_id) {
                $prestamos->where('libro_id', $libro_id);
            }
            ...
            $prestamos = $prestamos->paginate($perPage);
        ...
        try {
            $validador = Validator::make(
                ...
                [
                    ...
                    'fecha_prestamo' => 'La fecha de prestamo es obligatorio',
                    'fecha_devolucion_esperada.after_or_equal' => 'La fecha de devolución esperada debe ser posterior o igual a la fecha de préstamo.',
                    'estado' => 'El estado es Obigatorio',
                ]
            );
            if ($validador->fails()) {
    ...
    public function devolver(Request $request, $id)
    {
        try {
            ...
            if (
                ...
                $fechaDevolucionReal->greaterThan(Carbon::parse($prestamo->fecha_devolucion_esperada))
            ) {
                $diasRetraso = $fechaDevolucionReal->diffInDays(
                    Carbon::parse($prestamo->fecha_devolucion_esperada)
                );

                $multa = $diasRetraso * 5.00;
            }
